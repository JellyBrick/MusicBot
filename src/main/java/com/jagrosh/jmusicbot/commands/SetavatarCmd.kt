 ...
 */
class SetavatarCmd(bot: Bot) : Command() {
    ...
    override fun execute(event: CommandEvent) {
        val url: String? = if (event.args.isEmpty())
            if (!event.message.attachments.isEmpty() && event.message.attachments[0].isImage)
        ...
        if (s == null) {
            ...
            try {
                event.selfUser.manager.setAvatar(Icon.from(s)).queue(
                        { event.reply(event.client.success + " Successfully changed avatar.") },
                        { event.reply(event.client.error + " Failed to set avatar.") })
            } catch (e: IOException) {
