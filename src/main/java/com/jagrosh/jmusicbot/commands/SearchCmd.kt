 ...
 */
class SearchCmd(bot: Bot, private val searchingEmoji: String) : MusicCommand(bot) {

    private val builder: OrderedMenu.Builder

    init {
        this.name = "search"
        this.aliases = arrayOf("ytsearch")
    ...
    public override fun doCommand(event: CommandEvent) {
        ...
        event.reply(searchingEmoji + " Searching... `[" + event.args + "]`") { m -> bot.audioManager.loadItemOrdered(event.guild, "ytsearch:" + event.args, ResultHandler(m, event)) }
    }
        ...
        override fun playlistLoaded(playlist: AudioPlaylist) {
            ...
            while (i < 4 && i < playlist.tracks.size) {
                ...
                builder.addChoices("`[" + FormatUtil.formatTime(track.duration) + "]` [**"
                        + track.info.title + "**](https://youtu.be/" + track.identifier + ")")
                i++
            }
